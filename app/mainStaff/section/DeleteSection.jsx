...
const DeleteSection = () => {
  ...
  const handleOpen = () => {
    ...
    selectedSection
      ...
      : setMessage("You must select section to delete it");
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Send a Delete request with   sectionId in the header
  ...
  return (
    <form className="flex flex-col w-full  mx-auto">
      ...
      <Button
        type="submit"
        className="font-bold text-lg  text-white px-4 py-2 mt-4 rounded-lg w-[30%] mx-auto mb-5 transition-all duration-200 bg-red-600 hover:bg-red-700"
        data-dialog-target="animated-dialog"
        onClick={(e) => {
          e.preventDefault(); // This might be redundant but can be left as a safeguard
          handleOpen(); // Just toggle the modal, don't submit anything here
        }}
      >
      ...
      >
        <DialogBody>
          <div className="text-center bg-white rounded-lg p-5">
            ...
            <p className="text-red-600 font-semibold my-5 text-xl">
              {message}{" "}
            </p>
            <button
              onClick={(e) => {
                ...
                handleSubmit(e); // Proceed with your submit logic
              }}
              className={`bg-red-600 text-white py-3 px-6 font-bold rounded-lg uppercase shadow-md transition-all hover:bg-red-700 hover:shadow-lg active:opacity-75 ${
                !selectedSection ? "opacity-50 cursor-not-allowed" : ""
              }`} // Add classes for styling when disabled
              disabled={!selectedSection} // Disable button if selectedSection is null
            >
              Delete
            </button>
          </div>
