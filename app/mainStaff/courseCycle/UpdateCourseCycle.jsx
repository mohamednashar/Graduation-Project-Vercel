...
const UpdateCourseCycle = () => {
  ...
  const [selectedCourseCycle, setSelectedCourseCycle] = useState(null);
  ...
  const handleCourseSelectChange = async (selectedOption) => {
    setFormData({
      ...formData,
      courseId: selectedOption ? selectedOption.value : "",
    });
    // Fetch course cycles here
    await fetchCourseCycles(selectedOption.value, formData.groupId);
  };
  ...
  const fetchCourseCycles = async (courseId, groupId) => {
    console.log("course:", courseId, "group:", groupId);
    try {
      if (courseId && groupId) {
        const response = await axios.get(
  ...
  const handleCourseCycleSelectChange = (selectedOption) => {
    setSelectedCourseCycle(selectedOption);
  };
  ...
  const handleSubmit = async (event) => {
    ...
    // Get the course cycle ID from selectedCourseCycle
    const courseCycleId = selectedCourseCycle ? selectedCourseCycle.value : null;
    console.log(courseCycleId)
  
    if (!courseCycleId) {
      setErrorMessage("Please select a course cycle.");
      setSuccessMessage("");
      return;
    }
  
    try {
      // Send a PUT request with data in the body and course cycle ID in the header
      const response = await axios.put(
        `${API}CourseCycle/UpdateCourseCycle`,
        data,
        {
          headers: {
            Id: courseCycleId,
      ...
      <div className="bg-white p-5 rounded-lg shadow-md dark:bg-[#282828]">
        <div className="flex flex-col gap-5">
          ...
          <div className="flex gap-8">
            ...
            <div className="flex flex-col text-sm items-center w-full">
              <label htmlFor="SelectCourseCycle" className="mb-2">
                Course Cycle
              </label>
              <Select
                className="w-full"
                options={courseCycles.map((cycle) => ({
                  value: cycle.id,
                  label: cycle.name,
                }))}
                onChange={handleCourseCycleSelectChange}
                value={selectedCourseCycle}
              />
            </div>

            <div className="flex flex-col text-sm items-center w-full">
              <label htmlFor="SelectTitle" className="mb-2">
                Edit Title
              </label>
              <input
                type="text"
...
export default UpdateCourseCycle;
