  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        `${API}StudentSection/AddStudentsToSection`,
    ...
    status === 200 ?
      ...
      :
      (
        setFailedMessage("Failed to Add Students"),
        setSuccessMessage(""),
      ...
      <div style={containerStyle}>
        <div className="example-wrapper">
          <div className="example-header font-semibold">
            Selected students :<span id="selectedRows" className="whitespace-normal break-words"></span>
          </div>
        ...
        <div className="w-full text-center ">
          <button 
          className="font-bold text-lg text-center bg-[#66bfbf] text-white px-4 py-2 mt-4 rounded-lg w-[30%] mx-auto mb-5 transition-all duration-200 hover:bg-[#5eb1b1]"
          onClick={handleSubmit}
          >
            {" "}
            Add selected students to selected section
          </button>
        </div>
      </div>
      <Dialog
